...
const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const isAllLogsSelected = useMemo(() => {
    return (
      ...
      filteredLogs.every((log) => selectedLogIds.includes(String(log.id)))
    );
    ...
    <div className="admin-root">
      {showFaceError && (
        <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4">
          ...
          ></div>
          <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl relative w-full max-w-sm border border-indigo-50 animate-in zoom-in-95 duration-300">
            ...
            <h3 className="text-xl font-black text-slate-800 text-center uppercase tracking-tighter mb-2 leading-none">
              Wajah Tidak Dikenali
            </h3>
            <p className="text-slate-400 text-[10px] text-center mb-8 font-bold uppercase tracking-widest leading-relaxed">
              Tidak dapat mendeteksi struktur wajah pada foto ini. Pastikan
              pencahayaan cukup dan wajah menghadap kamera.
            </p>
            <button
              onClick={() => setShowFaceError(false)}
              className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-indigo-200 active:scale-95 transition-all"
            >
              Coba Foto Lagi
            </button>
          </div>
      ...
      {showErrorWidget && (
        <div className="fixed inset-0 z-[5000] flex items-center justify-center p-4">
          <div
            ...
            onClick={() => setShowErrorWidget(false)}
          ></div>
          <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl relative w-full max-w-sm border border-red-50 animate-in zoom-in-95 duration-300">
            ...
            <h3 className="text-xl font-black text-slate-800 text-center uppercase tracking-tighter mb-2">
              Save Failed
            </h3>
            <p className="text-slate-400 text-[10px] text-center mb-8 font-bold uppercase tracking-widest leading-relaxed">
              Pastikan konfigurasi atau URL server benar dan aktif.
            </p>
            <button
              onClick={() => setShowErrorWidget(false)}
              className="w-full py-4 bg-red-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest"
            >
              Tutup
            </button>
          </div>
        ...
        {activeTab === "dashboard" && (
          <div className="p-4 md:p-6 space-y-4 overflow-y-auto">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { l: "Total Logs", v: stats.total, c: "slate-800" },
                {
                  l: "Success Rate",
                  v: `${stats.successRate}%`,
                  c: "green-600",
                },
                { l: "Absen OK", v: stats.success, c: "indigo-600" },
                { l: "Storage Mode", v: storageMode, c: "orange-600" },
              ].map((s, i) => (
                <div
                  key={i}
                ...
                >
                  ...
                  <h4 className={`text-xl md:text-2xl font-black text-${s.c}`}>
                    {s.v}
                  </h4>
                </div>
              ))}
          ...
          <div className="admin-table-wrapper">
            <div className="bg-white py-2 border-b flex items-center justify-between">
              <div className="flex items-center">
                ...
                <p className="text-[9px] md:text-[11px] font-black text-slate-400 uppercase tracking-widest mr-3">
                  Aktivitas Terbaru
                </p>
                <div className="relative">
                  <button
                    onClick={() => setIsGroupMenuOpen(!isGroupMenuOpen)}
                    className={`flex items-center gap-1.5 px-2 py-1 rounded-md text-[8px] font-black uppercase transition-all ${
                      groupMode !== "none"
              ...
              <div className="pr-4 md:pr-6">
                {selectedLogIds.length > 0 && (
                  <button
                    ...
                    className="p-1 bg-red-50 text-red-600 rounded border border-red-100 animate-in zoom-in-95"
                  >
                    <svg
                    ...
                    <React.Fragment key={groupKey}>
                      {groupMode !== "none" && (
                        <tr className="bg-slate-50/80 backdrop-blur-sm sticky top-[36px] z-[30]">
                          <td
                            colSpan={4}
                            className="px-4 py-1.5 text-[8px] font-black text-indigo-500 uppercase tracking-widest border-y border-slate-100"
                          >
                            <span className="opacity-50 mr-1">Group:</span>{" "}
                          ...
                          >
                            ...
                            <td className="px-2 py-2 flex items-center gap-2">
                              <img
                                ...
                                className="w-7 h-7 md:w-10 md:h-10 rounded-lg object-cover border"
                                alt=""
                              />
                            ...
                            <td className="px-2 py-2 log-time-display">
                              ...
                              <span className="log-date-normal">
          ...
          <div className="p-4 md:p-8 space-y-4 overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                Data Karyawan ({users.length})
              </h3>
              <button
                onClick={() => setIsRegistering(true)}
                className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest"
              >
                + Baru
              </button>
            </div>
            ...
            {isRegistering ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <input
                      ...
                      className="w-full bg-white border rounded-lg p-3 text-xs outline-none focus:ring-2 focus:ring-indigo-500/20"
                    />
                    <input
                      type="text"
                      placeholder="ID Pegawai (NIK)"
                      value={empId}
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={!name || !empId || !photo || !password}
                        className="flex-1 bg-indigo-600 text-white py-3 rounded-xl font-black text-[9px] uppercase disabled:bg-slate-300"
                      >
                        Simpan
                      </button>
                      <button
                        onClick={() => setIsRegistering(false)}
                        className="px-4 border py-3 rounded-xl font-black text-[9px] uppercase"
                      >
                        Batal
                      </button>
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {filteredUsers.map((user) => (
                  <div
                    key={user.id}
                    className="bg-white border p-3 rounded-xl flex items-center gap-3 relative transition-all group"
                  >
                    <img
                    ...
                    <button
                      ...
                      className="text-slate-300 hover:text-red-500 p-1 opacity-40 group-hover:opacity-100"
                    >
                      <svg
                        className="w-3.5 h-3.5"
                        fill="none"
        ...
        {activeTab === "settings" && (
          <div className="p-4 md:p-8 overflow-y-auto space-y-6">
            ...
            <div className="max-w-md mx-auto bg-white border rounded-2xl p-6 shadow-sm space-y-4">
              <div className="text-center">
                ...
                <p className="text-[8px] text-slate-400 leading-tight">
                  Pilih sumber database (Dual Mode)
                </p>
              </div>
              ...
              {storageMode === "LOCAL" ? (
                <div className="space-y-3 pt-2">
                  <input
                    ...
                    className="w-full bg-slate-50 border rounded-lg px-4 py-3 text-[11px] font-bold outline-none"
                  />
                  <button
                  ...
                  >
                    {isSavingEndpoint ? "Connecting..." : "Update Local Server"}
                  </button>
                </div>
              ) : (
                <div className="space-y-3 pt-2">
                  <textarea
                    ...
                    className="w-full bg-slate-50 border rounded-lg px-4 py-3 text-[10px] font-mono h-32 outline-none resize-none"
                  />
                  <button
                    onClick={handleSaveSettings}
                    disabled={isSavingEndpoint}
                    className="w-full py-3 bg-orange-600 text-white rounded-xl font-black text-[9px] uppercase tracking-widest"
                  >
                    {isSavingEndpoint
                      ? "Connecting..."
                      : "Update Firebase Cloud"}
                  </button>
                  <p className="text-[7px] text-center text-slate-400">
                    Dapatkan di: Firebase Console lalu Project Settings lalu
                    General
                  </p>
                </div>
              )}
            ...
            <div className="max-w-md mx-auto bg-white border rounded-2xl p-6 shadow-sm space-y-6">
              ...
              <div className="space-y-1">
                <label className="text-[8px] font-black text-slate-400 uppercase block text-center">
                  Radius (Meter)
                </label>
                <input
                  type="number"
              ...
              <button
                ...
                className="w-full py-3 bg-slate-800 text-white rounded-xl font-black text-[9px] uppercase tracking-widest"
              >
                Gunakan Lokasi Saya
              </button>
            </div>
