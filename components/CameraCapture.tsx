...
const CameraCapture: React.FC<CameraCaptureProps> = ({ onCapture, label = "Take Photo", autoCapture = false }) => {
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      setError("Gagal mengakses kamera. Pastikan izin diberikan dan kamera tidak sedang digunakan aplikasi lain.");
    } finally {
      setIsStarting(false);
    }
    ...
    <div className="flex flex-col items-center gap-4 w-full">
      {error ? (
        <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-6 rounded-2xl text-center max-w-sm">
          ...
          <p className="text-[11px] font-bold uppercase tracking-wider mb-4">{error}</p>
          <button 
        ...
        <div className="relative w-full max-w-md bg-slate-950 rounded-[2.5rem] overflow-hidden shadow-2xl aspect-[4/3] border border-white/5 group">
          <video 
            ...
            playsInline 
            muted
            className={`w-full h-full object-cover transition-transform duration-700 ${facingMode === 'user' ? 'scale-x-[-1]' : 'scale-x-1'}`}
          />
          ...
          <button 
            ...
            disabled={isStarting}
            className={`absolute top-6 right-6 z-30 bg-black/40 backdrop-blur-xl p-3 rounded-2xl border border-white/10 text-white/90 hover:bg-indigo-600 active:scale-90 transition-all shadow-2xl ${isStarting ? 'opacity-50 cursor-not-allowed' : 'opacity-100'}`}
            title="Switch Camera"
          >
            <svg className={`w-5 h-5 ${isStarting ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 4h5l2 2h5a2 2 0 012 2v9a2 2 0 01-2 2H4a2 2 0 01-2-2V6a2 2 0 012-2z" />
               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 13l2-2 2 2m-2-2v5" />
            </svg>
          </button>
          ...
          {autoCapture && !isStarting && (
            <div className="absolute inset-0 pointer-events-none z-10">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                 <div className="w-56 h-72 border-2 border-white/10 rounded-[3rem] relative">
          ...
          {isStarting && (
            <div className="absolute inset-0 z-40 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center">
              <div className="flex flex-col items-center">
                ...
                <p className="text-[9px] font-black text-white uppercase tracking-[0.2em]">Switching Lens...</p>
              </div>
            </div>
          )}
          ...
          <div className="absolute inset-x-0 bottom-8 flex justify-center z-20">
            {!autoCapture ? (
              <button
                onClick={capture}
                disabled={isStarting}
                className="bg-white hover:bg-indigo-50 text-slate-900 font-bold p-5 rounded-full shadow-2xl transition-all active:scale-95 group/btn disabled:opacity-50"
              >
                <div className="w-10 h-10 rounded-full border-[6px] border-slate-900 group-hover/btn:scale-110 transition-transform"></div>
              </button>
            ) : (
              <div className="bg-black/60 backdrop-blur-xl px-6 py-2.5 rounded-full border border-white/10 flex items-center gap-3">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/90">
                  {facingMode === 'user' ? 'Face Recognition Active' : 'Object Scanning Active'}
                </span>
              </div>
            )}
          </div>
      ...
      <div className="flex items-center gap-2 opacity-50">
        ...
        <p className="text-slate-400 text-[9px] font-black uppercase tracking-[0.3em]">{label}</p>
        <div className="w-1 h-1 bg-slate-400 rounded-full"></div>
      </div>
